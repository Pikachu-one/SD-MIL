...
def five_scores_3n_classes(bag_labels, bag_predictions,plot_roc=False,test=False):
    '''
    fpr, tpr, threshold = roc_curve(bag_labels, bag_predictions, pos_label=1)
    fpr_optimal, tpr_optimal, threshold_optimal = optimal_thresh(fpr, tpr, threshold)
    # threshold_optimal=0.5
    ...
    auc_value = roc_auc_score(bag_labels, bag_predictions,multi_class='ovo')
    print('auc_value:',auc_value)
    #auc_value = roc_auc_score(bag_labels, bag_predictions)
    if plot_roc:
        # 绘制ROC曲线
        plt.figure()
        plt.plot(fpr, tpr, color='black', lw=2, label='ours_abmil__ROC curve (area = {:.3f})'.format(auc_value))
        plt.plot([0, 1], [0, 1], color='navy', lw=2, linestyle='--')
        plt.xlabel('False Positive Rate')
        plt.ylabel('True Positive Rate')
        plt.title('GastricCancer_roc_curve')
    ...
    fpr, tpr, thresholds = metrics.roc_curve(bag_labels.ravel(),bag_predictions.ravel())
    ...
    if plot_roc:
        ...
        plt.title('GastricCancer_roc_curve')
        plt.legend(loc="lower right")
        plt.savefig('JIANGFENG_roc_curve.png')
        plt.show()
    #this_class_label = np.array(bag_predictions)
    #this_class_label[this_class_label>=threshold_optimal] = 1
    #this_class_label[this_class_label<threshold_optimal] = 0
    ...
    #print('bag_labels:',bag_labels)
    #print('bag_predictions:',bag_predictions)
    bag_labels = np.argmax(bag_labels, axis=1)
    bag_predictions = np.argmax(bag_predictions, axis=1)
    #print('bag_labels:',bag_labels)
    #print('bag_predictions:',bag_predictions)
    accuracy = metrics.accuracy_score(bag_labels, bag_predictions)
    recall=metrics.recall_score(bag_labels,bag_predictions,average='weighted')
    precision=metrics.precision_score(bag_labels, bag_predictions, average='weighted')
    fscore=metrics.f1_score(bag_labels,bag_predictions,labels=[0,1,2],average='weighted')
    confusion_matrix=metrics.confusion_matrix(bag_labels,bag_predictions,labels=[0,1,2])
    classification_report=metrics.classification_report(bag_labels, bag_predictions,labels=[0,1,2])
    # accuracy = 1- np.count_nonzero(np.array(bag_labels).astype(int)- bag_predictions.astype(int)) / len(bag_labels)
    if test:
        return accuracy, auc_value, precision, recall, fscore,confusion_matrix,classification_report,bag_labels,bag_predictions
    else:
        return accuracy, auc_value, precision, recall, fscore,confusion_matrix,classification_report
