...
import argparse, os
from modules import attmil,clam,dsmil,transmil,mean_max,sdmil,attmil_ibmil,WIKG,rrt_origin
...
def main(args):
    ...
    if args.datasets.lower() == 'tcga':
        ...
        p = p[index]
        l = l[index]
    elif args.datasets == 'GastricCancer':
        label_path=os.path.join(args.dataset_root,'GastricCancer.csv')
        p, l = get_patient_label(label_path)
        index = [i for i in range(len(p))]
        random.shuffle(index)
        p = p[index]
    ...
    if args.cv_fold > 1:
        ...
        if args.datasets.lower() == 'camelyon16':
            #----------------------改----------------------------
            label_path=os.path.join(args.dataset_root,'label_c16.csv')
    ...
    for k in range(args.fold_start, args.cv_fold):
        ...
        if args.datasets == 'GastricCancer':
            if k !=2:
                continue 
...
def one_fold(args,k,ckc_metric,train_p, train_l, test_p, test_l,val_p,val_l):
    ...
    if args.datasets.lower() == 'camelyon16':
        ...
        train_set = GastricCancer(train_p[k],train_l[k],root=args.dataset_root,persistence=args.persistence,keep_same_psize=args.same_psize,is_train=True)
        test_set = GastricCancer(test_p[k],test_l[k],root=args.dataset_root,persistence=args.persistence,keep_same_psize=args.same_psize)
        if args.val_ratio != 0.:
            val_set = GastricCancer(val_p[k],val_l[k],root=args.dataset_root,persistence=args.persistence,keep_same_psize=args.same_psize)
        else:
            val_set = test_set
    ...
    if args.model == 'rrtmil':
        ...
        model = rrt_origin.RRTMIL(**model_params).to(device)
    elif args.model == 'SD-MIL':
        model_params = {
            'input_dim': args.input_dim,
            'n_classes': args.n_classes,
            'dropout': args.dropout,
            ...
            'crmsa_mlp':args.crmsa_mlp,
            'crmsa_heads':args.crmsa_heads,}
        model = sdmil.SDMIL(**model_params).to(device)
        ...
        model = WIKG.WiKG(dim_in=args.input_dim, dim_hidden=512, topk=6, n_classes=args.n_classes, agg_type='bi-interaction', dropout=0.3, pool='mean').to(device)    
    ...
    if args.early_stopping:
        early_stopping = EarlyStopping(patience=30 , stop_epoch=args.max_epoch ,save_best_model_stage=np.ceil(args.save_best_model_stage * args.num_epoch))
    else:
        early_stopping = None
    ...
    for epoch in range(epoch_start, args.num_epoch):
        ...
        print('\r Epoch [%d/%d] train loss: %.1E, test loss: %.1E, accuracy: %.6f, auc_value:%.6f, precision: %.6f, recall: %.6f, fscore: %.6f , time: %.6f(%.6f)' % 
        ...
        if auc_value > opt_auc and epoch >= args.save_best_model_stage*args.num_epoch:
            ...
            if not args.no_log:
                best_pt = {'model': model.state_dict()}
                torch.save(best_pt, os.path.join(args.model_path, 'fold_{fold}_model_best_auc.pt'.format(fold=k)))
        if test_loss < opt_loss and epoch >= args.save_best_model_stage*args.num_epoch:
            opt_loss=test_loss
            if not os.path.exists(args.model_path):
                os.mkdir(args.model_path)
            if not args.no_log:
                best_pt = {'model': model.state_dict()}
                torch.save(best_pt, os.path.join(args.model_path, 'fold_{fold}_model_best_loss.pt'.format(fold=k)))
    ...
    print('\n Optimal accuracy: %.6f ,Optimal auc: %.6f,Optimal precision: %.6f,Optimal recall: %.6f,Optimal fscore: %.6f' % (accuracy,auc_value,precision,recall,fscore))
    
    best_std = torch.load(os.path.join(args.model_path, 'fold_{fold}_model_best_loss.pt'.format(fold=k)))
    info = model.load_state_dict(best_std['model'])
    print(info)
    accuracy, auc_value, precision, recall, fscore,test_loss_log = test(args,model,test_loader,device,criterion)
    print('\n Optimal accuracy: %.6f ,Optimal auc: %.6f,Optimal precision: %.6f,Optimal recall: %.6f,Optimal fscore: %.6f' % (accuracy,auc_value,precision,recall,fscore))
        
    return [accuracy,precision,recall,fscore,auc,auc_value,test_loss_log]
    ...
    for i, data in enumerate(loader):
        ...
        with amp_autocast():
            ...
            if args.model in ('clam_sb','clam_mb','dsmil'):
                ...
                keep_num = patch_num   
            elif args.model_dowan_aggregate_model in('abmil','gatemil','transmil'): 
                train_logits=model(bag,return_attn=False,train=True)
                cls_loss,patch_num,keep_num = 0.,0.,0.
            elif args.model_dowan_aggregate_model in ('clam_mb','clam_sb'):
                train_logits,cls_loss,patch_num = model(bag,label,train=True)
                args.aux_alph=0.3
                args.cls_alpha=0.7
                keep_num=0
            elif args.model_dowan_aggregate_model in 'dsmil':
                train_logits,cls_loss,patch_num = model(bag,label,criterion,train=True)
                keep_num = patch_num
                args.cls_alpha = 0.5
                args.aux_alpha = 0.5
            else:
                train_logits = model(bag)
                cls_loss=keep_num = patch_num=0
            if args.loss == 'ce':
    ...
    with torch.no_grad():
        for i, data in enumerate(loader):
            ...
            if args.model == 'dsmil':
                ...
                test_logits,max_prediction = model(bag,label,criterion)
            else:
            ...
            if args.n_classes==2:
                ...
                if args.loss == 'ce':
                    test_loss = criterion(test_logits.view(batch_size,-1),label)  
                    if batch_size > 1:
                        raise KeyError('不存在n_classes=3且batch_size>1的情况,请单独编写代码')
                    else:
                        bag_logit.append(torch.softmax(test_logits,dim=1).cpu().squeeze().numpy())

                elif args.loss == 'bce':  
                    raise KeyError('不存在n_classes=3且loss=bec的情况,请单独编写代码')    
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dowan_aggregate_model', type=str, default='abmil' )
