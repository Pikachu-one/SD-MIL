...
class MHIM(nn.Module):
    def __init__(self, mlp_dim=512,mask_ratio=0,n_classes=2,temp_t=1.,temp_s=1.,dropout=0.25,act='relu',select_mask=True,select_inv=False,msa_fusion='vote',mask_ratio_h=0.,mrh_sche=None,mask_ratio_hr=0.,mask_ratio_l=0.,da_act='gelu',baseline='selfattn',head=8,attn_layer=0):
        ...
        #self.patch_to_emb = [nn.Linear(768, 512)]
        if act.lower() == 'relu':
        ...
        elif self.baseline == 'gatemil':
            self.online_encoder =DAttention(mlp_dim,da_act,gated=True)
        elif self.baseline == 'clam_sb':
            self.online_encoder=CLAM_SB(n_classes=2,dropout=0.25,act='rule')
        elif self.baseline == 'clam_mb':
            self.online_encoder=CLAM_MB(n_classes=2,dropout=0.25,act='rule')
...
if __name__ == "__main__":
    ...
    class MHIM_MIL(nn.Module):
        def __init__(self) -> None:
            ...
            self.teacher = MHIM(da_act='relu',baseline='selfattn').eval()
            self.teacher.requires_grad_(False)
            self.student = MHIM(da_act='relu',baseline='selfattn',mask_ratio_h=0.03,mask_ratio_hr=0.5,mask_ratio=0.,mask_ratio_l=0.8,cl_alpha=0.1,select_mask=True)
