...
from utils import eval_metric, get_cam_1d
from utils import get_patient_label,get_patient_label_no_cross_fold,get_kflod
...
def one_fold(params,k,train_p,train_l,test_p,test_l,val_p,val_l,epoch_step,writer):
        ...
        model_params = {
            ...
            'pos': params.pos,
            'pos_pos': params.pos_pos,
            'pool': params.pool,
            'peg_k': params.peg_k,
            'drop_path': params.drop_path,
            'n_layers': params.n_trans_layers,
            'n_heads': params.n_heads,
            'attn': params.attn,
            'da_act': params.da_act,
            'trans_dropout': params.trans_drop_out,
            'ffn': params.ffn,
            'mlp_ratio': params.mlp_ratio,
            'trans_dim': params.trans_dim,
            'epeg': params.epeg,
            'min_region_num': params.min_region_num,
            'qkv_bias': params.qkv_bias,
            'epeg_k': params.epeg_k,
            'epeg_2d': params.epeg_2d,
            'epeg_bias': params.epeg_bias,
            'epeg_type': params.epeg_type,
            'region_attn': params.region_attn,
            'peg_1d': params.peg_1d,
            'cr_msa': params.cr_msa,
            'crmsa_k': params.crmsa_k,
            'all_shortcut': params.all_shortcut,
            'crmsa_mlp':params.crmsa_mlp,
            'crmsa_heads':params.crmsa_heads,
            'model_dowan_aggregate_model':params.model_dowan_aggregate_model,
         }
        RRT=RRTMIL(**model_params).to(params.device)
        if params.init_type != 'none':
            ...
            if params.dataset.lower() == 'camelyon16':
                init_path=os.path.join('/home/liuzequn/multi_instance_learning_code/RRT-MIL-master/resnet50_camelyon16_no_flod/try/c16_r50_rrtmil_R-MSA_8_12_16_6_shortcut/fold_0_model_best_auc.pt')
                pre_dic = torch.load(init_path)
            elif params.dataset == 'GastricCancer':    
                init_path=os.path.join('/home/liuzequn/multi_instance_learning_code/RRT-MIL-master/resnet50_GastricCancer_flod/ablation_experiment/8_12_16_innovation_point_1_abmil2/fold_2_model_best_auc.pt')
                pre_dic = torch.load(init_path)
            if params.init_type=='fc':
                ...
                RRT.load_state_dict(pre_dic['model'],strict=False)
        ...
        log_dir = os.path.join(params.log_dir, params.name,'log.txt')
        log_dir_record = os.path.join(params.log_dir,params.name, 'record.txt')
        record_loss_best_txt_path = os.path.join(params.log_dir,params.name, 'record_loss_best.txt')
        # save_dir = os.path.join(params.log_dir,params.name, 'best_model.pth')
        save_dir = os.path.join(params.log_dir,params.name, 'fold_{fold}_best_auc_model.pth'.format(fold=k))
        save_dir2 = os.path.join(params.log_dir,params.name, 'fold_{fold}_best_loss_model.pth'.format(fold=k))
        ...
        # load data
        if params.dataset.lower() == 'camelyon16':
            SlideNames_train, FeatList_train, Label_train= reOrganize_mDATA_v2(train_p,train_l,dataset=params.dataset,dataset_root=params.dataset_root)
            if params.val_ratio != 0.:
                SlideNames_val, FeatList_val, Label_val= reOrganize_mDATA_v2(val_p,val_l,dataset=params.dataset,dataset_root=params.dataset_root)
                if params.always_test:
                    SlideNames_test, FeatList_test, Label_test = reOrganize_mDATA_v2(test_p,test_l,dataset=params.dataset,dataset_root=params.dataset_root)
            else:
                SlideNames_val, FeatList_val, Label_val = reOrganize_mDATA_v2(test_p,test_l,dataset=params.dataset,dataset_root=params.dataset_root)
        ...
        if params.early_stopping:
            earlystop = EarlyStopping(stop_epoch=130 ,patience=30 )
        else:
            earlystop = None
        ...
        for ii in range(params.EPOCH):
            ...
            time = train_attention_preFeature_DTFD(classifier=classifier, dimReduction=dimReduction, attention=attention, UClassifier=attCls, mDATA_list=(SlideNames_train, FeatList_train, Label_train), ce_cri=ce_cri,
                                                    optimizer0=optimizer_adam0, optimizer1=optimizer_adam1, RRT=RRT,epoch=ii, params=params, f_log=log_file, writer=writer, numGroup=params.numGroup, total_instance=params.total_instance, distill=params.distill_type)
            train_time_meter.update(time)
            print(train_time_meter.avg)
            print_log(f'>>>>>>>>>>> Validation Epoch: {ii}', log_file)
            auc_val,mF1_1,macc_1,mprec_1, mrecal_1,test_loss,gPred_1,gt_1  = test_attention_DTFD_preFeat_MultipleMean(classifier=classifier, dimReduction=dimReduction, attention=attention,
                                                            UClassifier=attCls, mDATA_list=(SlideNames_val, FeatList_val, Label_val), criterion=ce_cri,RRT=RRT, epoch=ii,  params=params, f_log=log_file, writer=writer, numGroup=params.numGroup_test, total_instance=params.total_instance_test, distill=params.distill_type)
            print_log(' ', log_file)
            if params.wandb:
                rowd = OrderedDict([
                    ("val_acc",macc_1),
            ...
            if ii > int(params.EPOCH*params.save_best_model_stage):
                if auc_val > best_auc:
                    best_auc = auc_val
                    best_epoch = ii
                    with open(log_dir_record, "w") as file:
                        file.write("test accuracy: %.3f" % macc_1)
                        file.write("test auc: %.3f" % best_auc)
                        file.write("test precision: %.3f" % mprec_1)
                        file.write("test recall: %.3f" % mrecal_1)
                        file.write("test fscore: %.3f\n" % mF1_1)
                        file.write("test epoch: %.3f\n" % best_epoch)
                        #file.write("test confusion_matrix: %s\n" % test_confusion_matrix_str)
                        #file.write("test classification_report: %s\n"  % test_classification_report)
                        for item in gt_1:
                            file.write(f'{item}\n')
                        file.write("模型预测\n")
                        for item in gPred_1:
            ...
            if test_loss.avg <= opt_loss and ii >= 0:
                opt_loss=test_loss.avg
                if params.isSaveModel:
                    tsave_dict = {
                        'model':RRT.state_dict(),
                        'classifier': classifier.state_dict(),
                        ...
                        'attention': attention.state_dict(),
                        'att_classifier': attCls.state_dict()}
                    torch.save(tsave_dict, save_dir2)
            scheduler0.step()
            scheduler1.step()
        ...
        tauc,tf1,tacc,tprec, trecal,_ ,bag_labels,bag_logit= test_attention_DTFD_preFeat_MultipleMean(classifier=classifier, dimReduction=dimReduction, attention=attention,
                                UClassifier=attCls, mDATA_list=(SlideNames_test, FeatList_test, Label_test), criterion=ce_cri, epoch=ii,  params=params, f_log=log_file, writer=writer, numGroup=params.numGroup_test, total_instance=params.total_instance_test, distill=params.distill_type)


        with open(record_loss_best_txt_path, "a") as file1:
            file1.write("test accuracy: %.6f" % tacc)
            file1.write("test auc: %.6f" % tauc)
            file1.write("test precision: %.6f" % tprec)
            file1.write("test recall: %.6f" % trecal)
            file1.write("test fscore: %.6f\n" % tf1)
            #file.write("test confusion_matrix: %s\n" % test_confusion_matrix_str)
            #file.write("test classification_report: %s\n"  % test_classification_report)
            for item in bag_labels:
                file1.write(f'{item}\n')
            file1.write("模型预测\n")
            for item in bag_logit:
                file1.write(f'{item}\n') 
        if params.wandb:
            rowd = OrderedDict([
                ("test_acc",tacc),
        ...
        return auc_val,mF1_1,macc_1,mprec_1, mrecal_1 ,best_val_auc_test
def main(params):
    ...
    if params.dataset.lower() == 'camelyon16':
        label_path=os.path.join('/home/liuzequn/multi_instance_learning_code/RRT-MIL-master/DTFD/label_c16.csv')
        ...
        label_path=os.path.join(params.dataset_root,'GastricCancer.csv')
        p, l = get_patient_label(label_path)
        index = [i for i in range(len(p))]
        random.shuffle(index)
        p = p[index]
    ...
    for k in range(0, params.cv_fold):
        if params.dataset == 'GastricCancer':
            if k !=2:
                continue
...
def test_attention_DTFD_preFeat_MultipleMean(mDATA_list, classifier, dimReduction, attention, UClassifier, RRT,epoch, criterion=None,  params=None, f_log=None, writer=None, numGroup=3, total_instance=3, distill='MaxMinS'):
    ...
    with torch.no_grad():
        ...
        for idx in range(numIter):
            ...
            for tidx, tfeat in enumerate(batch_feat):
                ...
                tfeat=RRT(tfeat.unsqueeze(0))
                tfeat=tfeat.squeeze(0)
    ...
    return auc_1,mF1_1.cpu(),macc_1.cpu(),mprec_1.cpu(), mrecal_1.cpu(),test_loss1,gPred_1,gt_1


def train_attention_preFeature_DTFD(mDATA_list, classifier, dimReduction, attention, UClassifier,  optimizer0, optimizer1, epoch, RRT,ce_cri=None, params=None,
    ...
    for idx in range(numIter):
        ...
        for tidx, (tslide, slide_idx) in enumerate(zip(tslide_name, tidx_slide)):
            ...
            tfeat_tensor=RRT(tfeat_tensor.unsqueeze(0))
            tfeat_tensor=tfeat_tensor.squeeze(0)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--name', default='resnet50_GastricCancer', type=str)
    parser.add_argument('--project', default='GastricCancer', type=str)
    parser.add_argument('--dataset_root', default='/home/liuzequn/CLAM-master/MHIM-MIL_Extracted_feature', type=str, help='dataset root path')
    ...
    parser.add_argument('--init_type', default='all', type=str, help='[none,fc,all]')
    ...
    parser.add_argument('--model_dowan_aggregate_model', type=str, default='dtfd' )
...
#CUDA_VISIBLE_DEVICES=3 python3 main.py  --dataset=camelyon16 --dataset_root=/home/liuzequn/CLAM-master/MHIM-MIL_Extracted_feature  --cv_fold=1  --pool=attn --n_trans_layers=2 --da_act=tanh --epeg_k=15 --crmsa_k=1 --all_shortcut --seed=2021 --model_dowan_aggregate_model='dtfd'
#CUDA_VISIBLE_DEVICES=3 python3 main.py --project=ablation_experiment --dataset=GastricCancer --dataset_root=/home/liuzequn/MHIM-MIL-master/MHIM-MIL-master/GastricCancer_MHIM-MIL_resnet_Extracted_feature  --cv_fold=3  --pool=attn --n_trans_layers=2 --da_act=tanh  --epeg_k=15 --crmsa_k=1 --all_shortcut --seed=2021 --model_dowan_aggregate_model='dtfd'