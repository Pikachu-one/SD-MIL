...
class RRTEncoder(nn.Module):
    def __init__(self,mlp_dim=512,pos_pos=0,pos='none',peg_k=7,attn='rmsa',region_num=8,drop_out=0.1,n_layers=2,n_heads=8,drop_path=0.,ffn=False,ffn_act='gelu',mlp_ratio=4.,trans_dim=64,epeg=True,epeg_k=15,region_size=0,min_region_num=0,min_region_ratio=0,qkv_bias=True,peg_bias=True,peg_1d=False,cr_msa=True,crmsa_k=3,all_shortcut=False,crmsa_mlp=False,crmsa_heads=8,need_init=False,**kwargs):
        ...
        #self.layer1 = TransLayer(dim=mlp_dim,head=n_heads,drop_out=drop_out,drop_path=drop_path,ffn=ffn,ffn_act=ffn_act,mlp_ratio=mlp_ratio,trans_dim=trans_dim,attn='ntrans',n_region=8,epeg=epeg,region_size=region_size,min_region_num=min_region_num,min_region_ratio=min_region_ratio,qkv_bias=qkv_bias,epeg_k=epeg_k,**kwargs)
        #self.pos_layer = PPEG2(dim=mlp_dim)
    ...
    def forward(self, x,coord=None): 
        ...
        if self.all_shortcut:
            #x = x1+x_shortcut2  
            x = x1+3*x_shortcut2+x2+x3                            #这个是效果最好的  
            #x = x1+x2+x3+x_shortcut2
        x = self.norm(x)
        if shape_len == 2:
            x = x.squeeze(0)
        elif shape_len == 4:
...
def distance_calculate(x,prototype):
    ...
    '''
    # 计算范数  
    x_norm = torch.norm(x_expanded, dim=2, keepdim=True)  # (P, N, 1)
    prototype_norm = torch.norm(prototype_expanded, dim=2, keepdim=True)  # (P, N, 1)
    # 避免除以零的情况
    x_norm[x_norm == 0] = 1
...
class RRTMIL(nn.Module):
    ...
    def forward(self, x,label=None,loss=None,train=False,return_attn=False,no_norm=False):
        ...
        #att为abmil
        #Prototype=self.pool_fn(x)
        #Prototype=Prototype.squeeze(0)
        #att为gatemmil
        Prototype,_=self.att(x)
        Prototype=Prototype.squeeze(0)
        #mean_prototype
        ...
        x_distance_feature=distance_calculate(x,Prototype)
        #add 
        #x=1.4*(self.fuse_weight_1*x+self.fuse_weight_2*x_distance_feature)
        ...
        elif self.model_dowan_aggregate_model=='dtfd':
            return x
        else:
            ValueError
...
#CUDA_VISIBLE_DEVICES=0 python3 main2.py --project=try2 --datasets=camelyon16 --dataset_root=/home/liuzequn/CLAM-master/MHIM-MIL_Extracted_feature --model_path=resnet50_camelyon16_no_flod --cv_fold=1 --model=rrtmil --pool=attn --n_trans_layers=2 --da_act=tanh --title=mahateen_distance_2pool --epeg_k=15 --crmsa_k=1 --all_shortcut --seed=2021
#CUDA_VISIBLE_DEVICES=1 python3 main2.py --project=try2 --datasets=camelyon16 --dataset_root=/home/liuzequn/CLAM-master/MHIM-MIL_Extracted_feature --model_path=resnet50_camelyon16_no_flod --cv_fold=1 --model=rrtmil --pool=attn --n_trans_layers=2 --da_act=tanh --title=oushi_distance_3instance_feature_cat_att_pool --epeg_k=15 --crmsa_k=1 --all_shortcut --seed=2021
#CUDA_VISIBLE_DEVICES=1 python3 main2.py --project=try2 --datasets=camelyon16 --dataset_root=/home/liuzequn/CLAM-master/MHIM-MIL_Extracted_feature --model_path=resnet50_camelyon16_no_flod --cv_fold=1 --model=rrtmil --pool=attn --n_trans_layers=2 --da_act=tanh --title=oushi_distance_2self_feature_att_pool_only_prototype --epeg_k=15 --crmsa_k=1 --all_shortcut --seed=2021
#CUDA_VISIBLE_DEVICES=0 python3 main2.py --project=ablation_experiment --datasets=camelyon16 --dataset_root=/home/liuzequn/CLAM-master/MHIM-MIL_Extracted_feature --model_path=resnet50_camelyon16_no_flod --cv_fold=1 --model=rrtmil --pool=attn --n_trans_layers=2 --da_act=tanh --title=8_12_16_oushi_distance_2_instance_feature_att_pool_cat --epeg_k=15 --crmsa_k=1 --all_shortcut --seed=2021
#CUDA_VISIBLE_DEVICES=0 python3 main2.py --project=ablation_experiment --datasets=camelyon16 --dataset_root=/home/liuzequn/CLAM-master/MHIM-MIL_Extracted_feature --model_path=resnet50_camelyon16_no_flod --cv_fold=1 --model=rrtmil --pool=attn --n_trans_layers=2 --da_act=tanh --title=8_12_16_oushi_distance_2_instance_feature_att_pool_sub --epeg_k=15 --crmsa_k=1 --all_shortcut --seed=2021